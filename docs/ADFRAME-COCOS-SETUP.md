...
| `cc.Widget` | 贴边对齐 | 仅稳态布局；详见下方 **H2.1 Widget 用法** |
...
`cc.Widget` **允许使用**，但只负责「相对父节点的稳态贴边 / 拉伸」。它与 tween、布局写回共用 **同一套 transform**，用法不对会表现为：拖了坐标又弹回、进场动画无效、Workspace 布局改 position 无效。

#### 允许做什么

| 场景 | 做法 |
|------|------|
| Scene Canvas 满屏 | Canvas 挂 Widget，四边对齐父级（模板常见 `_alignFlags: 45` = L+R+T+B） |
| Prefab 根贴满设计画布 | MainUI / CTA 根挂 Widget 四边对齐（相对实例父节点） |
| 顶栏 / 底栏 / 角标 | 只开需要的边（如 Top+Left），用 `top`/`left`/`right`/`bottom` 留白 |
| 代替 SafeArea | **不用** `cc.SafeArea`；按 720×1280 设计，用 Widget 边距手留安全区 |
| 宽度跟父 | 开 Left+Right（水平拉伸）；高度跟父则 Top+Bottom |

模板现状：MainUI / CTA 上 Widget 数量很少（满屏根级为主），**中间业务控件多数不用 Widget**，用固定 `UITransform` + 坐标。

#### 禁止 / 避免

| 禁止 | 原因 |
|------|------|
| 对**挂了 Widget 的同一节点**做 `position` / `scale` / `size` 的 tween 或 Animation | Widget 会在对齐时机把 transform 盖回去 |
| 布局编辑里拖「带 Widget 且对齐已锁定」的边，又期望自由 xy | Workspace 写回 position 可能被下次 align 覆盖 |
| 在倍率球、分数条等**需手调坐标**的叶子节点上挂 Widget | 与 Agent / 布局写回按名改 xy 冲突 |
| 用 Widget 做「从右侧滑入」一类动效 | 动效必须分层（见下） |
| 四边全开 + 再手写一套绝对坐标当主布局 | 双重真相，难维护 |

#### 动画必须分层（推荐结构）

```text
container   ← 只挂 Widget（定常态位置）；永不 tween position
  └── panel ← 动画 / 布局微调作用在这里；常态 local (0,0)
        └── 实际 Sprite / Label / Button …
```

- 进场：tween `panel` 从屏外到 `(0,0)`，不要 tween `container`
- 退场 / 点击反馈：同样只动 `panel`（或子节点）
- 需要改「停靠位置」时改 Widget 边距或 container 的对齐，而不是改 panel 的稳态坐标

#### 与布局写回 / AI 的关系

- Workspace 布局写回主要改节点 **position** 与 **UITransform 尺寸**
- 目标节点若带 Widget：
  - **仅对齐、不参与手拖的满屏根**：一般不要在布局 Tab 里当可拖控件改 xy
  - **可拖业务节点**：优先 **去掉 Widget**，改用固定坐标；或保证只改未锁定轴
- Agent 换皮 / 改球顺序：动的是叶子节点坐标或 sibling，**不要**给这些节点新加 Widget

#### AI / 人工自检

- [ ] 挂 Widget 的节点是否只做贴边，不做动画目标？
- [ ] 动画目标是否落在 Widget 节点的**子节点**上？
- [ ] 需要 Workspace 自由拖动的节点是否**没有** Widget？
- [ ] 未引入 `cc.SafeArea`，边距是否用 Widget 或固定坐标表达？
...
- [ ] **Widget**：仅稳态贴边；动画/手拖节点与 Widget 分层；可拖叶子无 Widget
